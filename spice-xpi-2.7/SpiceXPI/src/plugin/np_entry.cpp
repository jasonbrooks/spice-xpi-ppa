...
void SetUpQD()
{
    ...
    // Memorize the plugins resource file refnum for later use.
    gResFile = CurResFile();
    ...
    if (result == noErr)
    {
        ...
        result =  GetDiskFragment(infoRec.processAppSpec, 0L, 0L, infoRec.processName,
                                  kReferenceCFrag, &connID, static_cast<Ptr *>(&mainAddr), errName);
    }
    ...
    if (result == noErr)
    {
        ...
        result = FindSymbol(connID, "\pqd", static_cast<Ptr *>(&gQDPtr), &symClass);
    }
    else
    {
        // The app is 68K, so use its A5 to compute the address
        // of its QuickDraw globals.
        gQDPtr = static_cast<QDGlobals *>(*(static_cast<long *>(SetCurrentA5())) - (sizeof(QDGlobals) - sizeof(GrafPtr)));
    }
